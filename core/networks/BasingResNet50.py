...
from keras.layers import Conv2D, Dense, Dropout, Activation, add, Input 
...
def Resnet50_model1(input_shape=None):
...
def Resnet50_locate(input_shape=None):
    #input_shape = (512, 512, 3)
    model_base = ResNet50(include_top=False, input_shape=input_shape, weights='imagenet', pooling='max')
    #feature_model# (16,16,2048)
    x = model_base.get_layer("activation_49").output
...
def ResNet50_classify(input_shape=None, frozen_layers=79):
    """二分类ResNet50
...
def ResNet50_model2(input_shape=None, frozen_layers=79):
    """二分类ResNet50
    :param input_shape: 输入尺寸
    :param frozen_layers: 冻结层数
    :return: 返回建立好的模型
